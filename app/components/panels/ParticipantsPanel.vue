...
const { roomId, userId, roomName, participants, publishers, participantCount, displayName, isMuted, isVideoOff } =
...
const meetingId = computed(() => roomId.value || '')

// ✅ Get participants/publishers as arrays (Map => Array)
...
// ✅ Participants excluding self + excluding users that are already in publishers list
const otherParticipants = computed(() => {
  const self = userId.value
  const pubUserIds = new Set(publishersArr.value.map(p => p.user_id))
  return participantsArr.value.filter(p => p.user_id !== self && !pubUserIds.has(p.user_id))
})
...
async function ensureInviteLink() {
  ...
  try {
    ...
    console.error('[ParticipantsPanel] createInvitation failed:', e)
...
/**
 * ✅ FIX: bouton “Invite participants”
 * - On ouvre le modal immédiatement
 * - On évite stop/prevent (parfois ça casse selon layout / overlay)
 * - On log pour vérifier que le click arrive bien
 */
async function openInviteModal() {
  console.log('[ParticipantsPanel] Invite button clicked ✅')

  showInviteModal.value = true

  // On génère le lien ensuite, sans bloquer l'ouverture du modal
  // (si l’API lag, le modal s’ouvre quand même)
...
async function sendInviteEmail() {
  ...
  const subject = `TrueGather invite — ${roomName.value || 'Meeting'}`
  ...
  try {
    ...
    console.warn('[ParticipantsPanel] sendInviteEmail failed, fallback to mailto:', e)
...
<template>
  <div class="flex flex-col h-full bg-bg-secondary">
    ...
    <div class="flex-1 overflow-y-auto p-2">
      ...
      >
        ...
        <div class="flex items-center gap-1">
          <div v-if="isMuted" class="p-1 rounded bg-danger/20" title="Muted">
            <Icon name="heroicons:microphone-solid" class="w-4 h-4 text-danger" />
          </div>
          <div v-if="isVideoOff" class="p-1 rounded bg-danger/20" title="Camera off">
      ...
      <!-- Other publishers -->
      <div
        v-for="publisher in publishersArr"
      ...
      <!-- Other non-publishing participants -->
      <div
      ...
      >
        ...
        <div class="flex-1 min-w-0">
          <p class="font-medium text-text-primary truncate">{{ p.display }}</p>
        </div>
      </div>

      <!-- Empty state -->
      <div
        v-if="otherParticipants.length === 0 && publishersArr.length === 0"
    ...
    <!-- Footer actions -->
    <div class="p-4 border-t border-border relative z-50 pointer-events-auto">
      <!-- ✅ FIX: bouton robuste -->
      <button
        type="button"
        class="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-bg-elevated hover:bg-bg-hover text-text-primary transition-colors cursor-pointer pointer-events-auto"
  ...
  <!-- ✅ FIX: Les modals en ClientOnly (Teleport/document safe) -->
  <ClientOnly>
    <!-- MAIN INVITE MODAL -->
      ...
      <div class="space-y-4">
        ...
        <div class="flex gap-2">
          <BaseButton
            variant="secondary"
            class="flex-1"
            :disabled="isCreatingInvite || isSendingEmail"
            @click="copyInviteLink"
          >
            Copy link
          </BaseButton>
