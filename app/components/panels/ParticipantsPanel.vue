...
const otherParticipants = computed(() => {
  ...
  const pubUserIds = new Set(publishers.value.map(p => p.user_id))
  return participants.value.filter(p => p.user_id !== selfId && !pubUserIds.has(p.user_id))
})
...
<template>
  <div class="flex flex-col h-full bg-bg-secondary">
    ...
    <div class="flex-1 overflow-y-auto p-2">
      ...
      <!-- Empty state -->
      <div
        v-if="otherParticipants.length === 0 && publishers.length === 0"
