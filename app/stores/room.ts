...
import type {
  ...
  SignalingMessage,
  JoinRoomRequest, // ✅ on utilise le bon type backend
} from '~/types'
...
export const useRoomStore = defineStore('room', () => {
   ...
   * - Host:  { creator_key }
   */
  async function joinRoom(id: string, display: string, access: Omit<JoinRoomRequest, 'display'>): Promise<void> {
    ...
    try {
      ...
      // ✅ Join via REST API to get credentials (ws_url + jwt token + ice servers)
      // IMPORTANT: on envoie les bons champs: invite_token/invite_code OU creator_key
      ...
      // Send join_room message (signaling layer)
      const joinResult = await signaling.sendRequest<SignalingMessage<JoinedPayload>>('join_room', {
        room_id: id,
        display,
      })

      // Process existing publishers (backend sends "publishers")
      ...
      // On remonte le vrai message backend si dispo (Invalid invitation code, etc.)
      console.error('[RoomStore] Failed to join:', error)
      toastStore.error(error?.data?.error || 'Failed to join room')
      await cleanup()
      throw error
    } finally {
  ...
  function setupSignalingHandlers(): void {
    ...
    signaling.on('remote_candidate', async (message) => {
      ...
      // Candidate pour un subscriber feed
      if (payload.feed_id) {
        ...
        // Candidate pour publisher local
  ...
  async function startPublishing(): Promise<void> {
    ...
    try {
      ...
      // Now subscribe to any existing publishers
  ...
  async function subscribeToPublisher(feedId: string, display: string, publisherUserId: string): Promise<void> {
    try {
      ...
      const answerSdp = await subscriber.subscribe(feedId, offerResponse.payload.sdp, display, publisherUserId)
